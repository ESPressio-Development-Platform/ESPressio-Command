...
enum class CommandLedgerSlotState : std::uint8_t {
    Empty=0,Started=1,CompletedNoResult=2,HandlerFailed=3,Indeterminate=4,CompletedResultRetained=5
};
constexpr bool IsTerminalLedgerState(CommandLedgerSlotState state) noexcept {
    return state==CommandLedgerSlotState::CompletedNoResult || state==CommandLedgerSlotState::HandlerFailed ||
           state==CommandLedgerSlotState::Indeterminate || state==CommandLedgerSlotState::CompletedResultRetained;
}
...
template<class T> class CommandExecutionLedger final {
    ...
    static constexpr bool PersistentResponseResults=!std::is_same_v<Response,NoCommandResponse> && ResultRetention::Persistent;
    static_assert(OriginCapacity>0 && WindowCapacity>0 && OriginCapacity<=UINT16_MAX && WindowCapacity<=UINT16_MAX);
    static_assert(!std::is_same_v<Response,NoCommandResponse> || !ResultRetention::Persistent,
    ...
    enum class ResultProbeStatus : std::uint8_t { NotFound, Valid, Corrupt, StorageFailure };
    struct ResultProbe final { ResultProbeStatus Status=ResultProbeStatus::NotFound;std::size_t PayloadBytes=0;System::DeviceRuntimeIdentity Executor{}; };
public:
    ...
    static constexpr std::size_t PersistentResultScratchBytes=PersistentResponseResults ? ResultRecordHeaderBytes+ResultRetention::MaximumRetainedBytes+4 : 0;
    static constexpr std::size_t MaximumPersistentResults=PersistentResponseResults ? ResultRetention::MaximumRetainedResults : 0;
    static constexpr std::size_t MaximumPersistentResultBytes=PersistentResponseResults ? ResultRetention::MaximumRetainedBytes : 0;
...
private:
    ...
    CommandPersistenceBinding _binding{};System::DeviceIdentifier _localDevice{};Primitive::ContractFingerprint _contract{};
    std::size_t _maximumResultPayloadBytes=0,_retainedResultCount=0,_retainedResultBytes=0,_reservedResultCount=0,_reservedResultBytes=0;
    bool _initialized=false;
    static constexpr std::uint8_t Magic[4]={'C','M','D','L'};static constexpr std::uint8_t ResultMagic[4]={'C','M','D','R'};
    static constexpr std::uint16_t RecordVersion=1,ResultRecordVersion=1;
...
public:
    ...
    CommandRuntimeStatus Bind(CommandPersistenceBinding) noexcept;bool HasBinding() const noexcept;
    ...
    bool PersistPersistentResult(const CommandExecutionKey&,const System::DeviceRuntimeIdentity&,const Response&) noexcept;
    bool RetirePersistentResult(const CommandExecutionKey&) noexcept;
    std::size_t RetainedResultCount() const noexcept;std::size_t RetainedResultBytes() const noexcept;
...
public:
    using Reservation=NoCommandLedgerReservation;static constexpr bool UsesPersistentResults=false;
    ...
    bool ReservePersistentResult(const CommandExecutionKey&) noexcept { return true; }
    void AbandonPersistentResult(const CommandExecutionKey&) noexcept {}
    bool PersistPersistentResult(const CommandExecutionKey&,const System::DeviceRuntimeIdentity&,const typename T::ResponseType&) noexcept { return false; }
    bool RetirePersistentResult(const CommandExecutionKey&) noexcept { return true; }
};
...
public:
    ...
    bool ReservePersistentResult(const CommandExecutionKey& key) noexcept { return _ledger.ReservePersistentResult(key); }
    void AbandonPersistentResult(const CommandExecutionKey& key) noexcept { _ledger.AbandonPersistentResult(key); }
    bool PersistPersistentResult(const CommandExecutionKey& key,const System::DeviceRuntimeIdentity& executor,const typename T::ResponseType& response) noexcept { return _ledger.PersistPersistentResult(key,executor,response); }
    bool RetirePersistentResult(const CommandExecutionKey& key) noexcept { return _ledger.RetirePersistentResult(key); }
