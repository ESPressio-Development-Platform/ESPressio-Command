...
public:
    ...
    CommandValue& operator=(std::string value) {
    ...
    T As() const {
        ...
        if constexpr (std::is_same_v<Target, CommandValue>) {
            ...
            if (IsNull()) {
                throw std::invalid_argument("Null command value cannot be converted to string");
            }
            return ToString();
            ...
            if (std::holds_alternative<int64_t>(value_)) {
                const int64_t value = std::get<int64_t>(value_);
                if (value == 0 || value == 1) return value == 1;
                throw std::invalid_argument("Expected boolean-compatible integer 0 or 1");
            }
            if (std::holds_alternative<uint64_t>(value_)) {
                const uint64_t value = std::get<uint64_t>(value_);
                if (value == 0 || value == 1) return value == 1;
                throw std::invalid_argument("Expected boolean-compatible integer 0 or 1");
            }
            if (std::holds_alternative<std::string>(value_)) {
            ...
            if (std::holds_alternative<int64_t>(value_)) {
                ...
                if constexpr (std::is_signed_v<Target>) {
                    ...
                    if (!value.empty() && value.front() == '-') {
                        throw std::out_of_range("Unsigned integer cannot be negative: " + value);
                    }
