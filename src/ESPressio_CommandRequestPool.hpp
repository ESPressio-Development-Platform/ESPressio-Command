...
template<class T> class CommandRequestLease final {
    ...
    explicit CommandRequestLease(Control* c) noexcept : _control(c) {}
    ...
    void Release() noexcept {
        if(_control && _control->References.fetch_sub(1,std::memory_order_acq_rel)==1) _control->DestroyAndRelease(_control);
        _control=nullptr;
    }
public:
    ...
    CommandRequestLease& operator=(const CommandRequestLease& other) noexcept {
        if(this==&other) return *this; Release(); _control=other._control;
        if(_control) _control->References.fetch_add(1,std::memory_order_relaxed); return *this;
    }
    CommandRequestLease(CommandRequestLease&& other) noexcept : _control(std::exchange(other._control,nullptr)) {}
    CommandRequestLease& operator=(CommandRequestLease&& other) noexcept {
        if(this!=&other){ Release(); _control=std::exchange(other._control,nullptr); } return *this;
    }
...
template<class T> class CommandRequestPool final {
    ...
    void Return(std::size_t index) noexcept {
        {
            ...
            auto& slot=_slots[index]; slot.ControlBlock.Payload=nullptr; slot.Reserved=false; --_occupied;
        }
    ...
    static void Destroy(Control* control) noexcept {
        ...
        auto& slot=pool._slots[control->Index]; std::launder(reinterpret_cast<T*>(slot.Storage))->~T();
        pool.Return(control->Index);
    }
public:
    class Reservation final {
        CommandRequestPool* _pool=nullptr; std::size_t _index=0;
        friend class CommandRequestPool;
        Reservation(CommandRequestPool* p,std::size_t i) noexcept:_pool(p),_index(i){}
    public:
        Reservation() noexcept=default; Reservation(const Reservation&)=delete; Reservation& operator=(const Reservation&)=delete;
        Reservation(Reservation&& other) noexcept:_pool(std::exchange(other._pool,nullptr)),_index(other._index){}
        Reservation& operator=(Reservation&&)=delete;
        ~Reservation(){ if(_pool) _pool->Return(_index); }
        explicit operator bool() const noexcept { return _pool!=nullptr; }
        template<class... Args> Lease Construct(const CommandRequestFacts& facts,Args&&... args) {
            if(!_pool) std::terminate(); auto& slot=_pool->_slots[_index];
            auto* value=new(slot.Storage) T(std::forward<Args>(args)...);
            slot.ControlBlock.Facts=facts; slot.ControlBlock.Payload=value;
            ...
            slot.ControlBlock.References.store(1,std::memory_order_release); _pool=nullptr;
    ...
    CommandRequestPool() noexcept {
        ...
        for(std::size_t i=0;i<_slots.size();++i){ auto& c=_slots[i].ControlBlock;c.Pool=this;c.Index=i;c.DestroyAndRelease=&Destroy; }
    }
    ~CommandRequestPool(){ if(_occupied.load()!=0) std::terminate(); }
    CommandRequestPool(const CommandRequestPool&)=delete; CommandRequestPool& operator=(const CommandRequestPool&)=delete;
    void BindCapacityWake(void* owner,void(*wake)(void*) noexcept) noexcept { std::lock_guard<System::Synchronization::Mutex> l(_mutex);_wakeOwner=owner;_capacityChanged=wake; }
    Reservation TryReserve() noexcept {
        ...
        for(std::size_t i=0;i<_slots.size();++i){ auto& s=_slots[i];
            if(s.Reserved || s.ControlBlock.Generation==std::numeric_limits<std::uint64_t>::max()) continue;
            s.Reserved=true;++s.ControlBlock.Generation;++_occupied;return Reservation(this,i);
        } return {};
    }
