...
public:
    explicit Runtime(RuntimeConfiguration configuration={}) noexcept
    ...
    ~Runtime() {
        if(Shutdown()!=CommandRuntimeStatus::Success) {
            std::terminate();
        }
    }

    template<class T,class TOwner,class TMethod>
    CommandRuntimeStatus BindHandler(
        TOwner& owner,
        TMethod method) noexcept {
    ...
    template<class T>
    CommandRuntimeStatus BindPersistence(
        Persistence::IAtomicRecordStore& store,
    ...
    template<class T>
    CommandRuntimeStatus BindPersistence(
        Persistence::IAtomicRecordStore& ledgerStore,
        const Persistence::AtomicRecordKey& ledgerKey,
    ...
    CommandRuntimeStatus Initialize(Primitive::TypeDirectoryView directory) {
        ...
        if(!_configuration.ExecutionLane.StackSize) {
            return CommandRuntimeStatus::InvalidConfiguration;
        }

        std::size_t responseSlots=0;
        for(const auto& common:directory) {
            if(common.Key.Family!=CommandFamilyId) {
                continue;
            }
            const auto* descriptor=GetCommandTypeDescriptor(common);
            if(!descriptor ||
               !descriptor->HasHandler ||
               !descriptor->HasPersistence ||
               !descriptor->BindPersistence ||
               !descriptor->Initialize ||
               !descriptor->ValidateStart ||
               !descriptor->StartValidated ||
               !descriptor->CloseAdmissions ||
               !descriptor->Shutdown ||
               !descriptor->RollbackInitialization ||
            ...
            if(descriptor->MaximumPendingResponses>SIZE_MAX-responseSlots) {
                return CommandRuntimeStatus::InvalidConfiguration;
            }
            responseSlots+=descriptor->MaximumPendingResponses;
            if(descriptor->MaximumLiveInstances<
               descriptor->MaximumPendingExecutions+descriptor->ExecutionLaneCount) {
                return CommandRuntimeStatus::InvalidDirectory;
            }
            if(descriptor->MaximumPendingResponses &&
               descriptor->MaximumPendingResponses<
                   descriptor->MaximumPendingExecutions+descriptor->ExecutionLaneCount) {
                return CommandRuntimeStatus::InvalidDirectory;
            }
        }
        ...
        if(responseSlots &&
           (!_configuration.ResponseRouter ||
            _configuration.ResponseRouter.Capacity()<responseSlots)) {
            return CommandRuntimeStatus::InvalidConfiguration;
        }

        for(const auto& common:directory) {
            if(common.Key.Family==CommandFamilyId &&
               !GetCommandTypeDescriptor(common)->HasHandler()) {
        ...
        for(const auto& common:directory) {
            ...
            const auto* descriptor=GetCommandTypeDescriptor(common);
            if(descriptor->MaximumPendingResponses) {
                const auto bind=descriptor->BindResponseRouter(
                    _configuration.ResponseRouter);
            ...
            const auto status=descriptor->Initialize(
                _configuration.ExecutionLane,
                &_running);
    ...
    CommandRuntimeStatus Start() noexcept {
        if(!_initialized) {
            return CommandRuntimeStatus::NotInitialized;
        }
        if(_stopping) {
            return CommandRuntimeStatus::Stopping;
        }
        ...
        for(const auto& common:_directory) {
            if(common.Key.Family==CommandFamilyId &&
               !GetCommandTypeDescriptor(common)->ValidateStart()) {
                return CommandRuntimeStatus::InvalidConfiguration;
            }
        }
        ...
        for(const auto& common:_directory) {
            if(common.Key.Family==CommandFamilyId) {
                GetCommandTypeDescriptor(common)->StartValidated();
            }
        }
    ...
    CommandRuntimeStatus Shutdown() noexcept {
        if(!_initialized) {
            if(_routerInitialized) {
                ...
                if(status!=Task::TaskExecutionStatus::Success) {
                    return CommandRuntimeStatus::JoinFailed;
                }
            }
            return CommandRuntimeStatus::Success;
        }
        ...
        for(const auto& common:_directory) {
            ...
            if(GetCommandTypeDescriptor(common)->Shutdown()!=
               CommandRuntimeStatus::Success) {
                joined=false;
            }
        }
    ...
    CommandRuntimeResourceProfile GetResourceProfile() const noexcept {
        ...
        profile.ResponseRouterCapacity=
            _configuration.ResponseRouter
                ? _configuration.ResponseRouter.Capacity()
                : 0;
        profile.ResponseRouterQueueBytes=
            profile.ResponseRouterCapacity*
            sizeof(Detail::CommandResponseRouteWork);
