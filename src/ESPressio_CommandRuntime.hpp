...
struct CommandRuntimeResourceProfile final {
    std::size_t CoordinatorBytes=0,CoordinatorAlignment=1,TypeCount=0,TypeResidentBytes=0,ExecutionContexts=0;
    std::size_t DestinationResponseSlots=0,ResponseRouterCapacity=0,ResponseRouterQueueBytes=0;
    std::uint32_t StackBytesPerLane=0;
};
class Runtime final {
    RuntimeConfiguration _configuration;Primitive::TypeDirectoryView _directory;std::atomic<bool> _running{false};
    ...
    CommandRuntimeStatus Initialize(Primitive::TypeDirectoryView directory){
        ...
        for(const auto& common:directory){
            ...
            if(!descriptor||!descriptor->HasHandler||!descriptor->HasPersistence||!descriptor->BindPersistence||
               !descriptor->Initialize||!descriptor->ValidateStart||!descriptor->StartValidated||!descriptor->CloseAdmissions||
               !descriptor->Shutdown||!descriptor->RollbackInitialization||!descriptor->BindResponseRouter)
                return CommandRuntimeStatus::InvalidDirectory;
            if(descriptor->Tier==CommandTier::Transmissible&&!descriptor->HasPersistence()) return CommandRuntimeStatus::MissingPersistence;
            ...
            if(descriptor->MaximumLiveInstances<descriptor->MaximumPendingExecutions+descriptor->ExecutionLaneCount) return CommandRuntimeStatus::InvalidDirectory;
        ...
        if(responseSlots&&(!_configuration.ResponseRouter||_configuration.ResponseRouter.Capacity()<responseSlots)) return CommandRuntimeStatus::InvalidConfiguration;
        for(const auto& common:directory)
...
private:
    void RollbackPrepared() noexcept {
        std::size_t visited=0;
        for(const auto& common:_directory){
            if(common.Key.Family!=CommandFamilyId) continue;if(visited++==_preparedTypes) break;(void)GetCommandTypeDescriptor(common)->RollbackInitialization();
        }
        _preparedTypes=0;
