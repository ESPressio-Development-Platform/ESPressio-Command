...
template<class T> constexpr bool ValidateCommandType() noexcept {
    static_assert(std::is_same_v<std::remove_cv_t<decltype(T::TypeId)>, CommandTypeId>,
                  "Command TypeId must be strong CommandTypeId");
    ...
    static_assert(IsExecutionAdmissionPolicy<typename T::ExecutionAdmissionPolicy>::value,
                  "Invalid Command ExecutionAdmissionPolicy");
...
template<class T> class CommandTypeRuntime final {
    ...
    void ExecuteLane(WorkItem& item) noexcept {
        ...
        const CommandExecutionContext context{
            item.Request.Facts().Key, item.Request.Facts().OriginRequestTime, *executor};
...
public:
    ...
    template<class TOwner, class TMethod>
    ...
    CommandRuntimeStatus Initialize(Task::TaskExecutionConfiguration config,
        ...
        try {
            _capacityChanged = provider->CreateBinarySignal(false);
        } catch (...) {
            return CommandRuntimeStatus::StorageUnavailable;
        }
        if (!_capacityChanged) return CommandRuntimeStatus::StorageUnavailable;
        _pool.BindCapacityWake(this, [](void* owner) noexcept {
            static_cast<CommandTypeRuntime*>(owner)->Wake();
        });

        std::size_t initialized = 0;
        for (; initialized < LaneCount; ++initialized) {
            const auto result = _lanes[initialized]
                .template Initialize<CommandTypeRuntime,
                                     &CommandTypeRuntime::ExecuteLane,
                                     &CommandTypeRuntime::OnLaneReleased>(*this, config);
        ...
        _captureTime = capture;
        _familyRunning = familyRunning;
    ...
    CommandRuntimeStatus RollbackInitialization() noexcept {
        std::lock_guard<System::Synchronization::Mutex> lock(_admission);
        if (_phase != Phase::Prepared || _pool.Occupied() != 0 || !_pending.Empty()) {
        ...
        _familyRunning = nullptr;
        _phase = Phase::Uninitialized;
        return CommandRuntimeStatus::Success;
    }

    template<bool Blocking, class... Args>
    CommandSubmissionResult SubmitLocal(Args&&... args) {
        ...
        if (_familyRunning && !_familyRunning->load(std::memory_order_acquire)) {
            return {CommandSubmissionStatus::Stopping, {}};
        }
        ...
        for (;;) {
            ...
            if (_phase != Phase::Running ||
                (_familyRunning && !_familyRunning->load(std::memory_order_acquire))) {
                lock.unlock();
                Wake();
            ...
            if constexpr (!std::is_same_v<Response, NoCommandResponse>) {
                response = _responses.TryReserve(key);
                if (!response) {
                    if constexpr (Blocking && policyWait) {
                        lock.unlock();
                        (void)_capacityChanged->Wait();
                        continue;
                    }
                ...
                try {
                    ...
                    if (_pending.Empty()) {
                        for (std::size_t i = 0; i < LaneCount; ++i) {
                            ...
                            if (result.Status == Task::TaskExecutionStatus::GenerationExhausted) {
